...
<!-- (empty — all triaged tasks done 2026-07-05) -->
...
- [x] **#30** větší počet válců a větší objem znamená větší spolehlivost — DONE 2026-07-05: new payload-only (not canonical) "Spolehlivost" column, `build/build_data.py::reliability_score(engine_volume, cylinder_count)` — coarse heuristic per the owner's rule of thumb, not empirical data: volume bins (≤1.0→1, ≤1.3→2, ≤1.6→3, ≤2.0→4, >2.0→5) blended with cylinder bins (≤3→1, 4→3, 5→4, ≥6→5) via `round(mean(...))` when both known, clamped 1–5. #24's "Počet válců" is confirmed empty in practice today (sauto detail API carries no cylinder field; no other source has one either), so the score **must and does degrade to volume-only** — verified live: the current build has zero populated "Počet válců" rows and every matched ICE row still gets a volume-only score. `add_reliability_column()` gates ICE-only (`Typ == "Spalovací"`) and inserts right after "Počet válců"; applied to ALL ICE rows with a volume, not just `Spárováno == "Ano"` (deviation from the original scoping note — simpler and covers Nejisté/Ne rows too, which still have a real engine). Numeric float64 in `PAYLOAD_NUMERIC_COLS` (sortable/filterable + conditional coloring, consistent with other numeric columns) rather than a string column. `site/app.js` COL_CONFIG: narrow numeric column next to "Počet válců", Czech tooltip explaining the heuristic and its volume-only degradation. Tests: `tests/test_build_data.py::ReliabilityScoreTest` (11 cases: bins, blend rounding, blank-on-missing-volume, EV always blank, position), `tests/test_data_integrity.py` (range 1–5, EV always blank, no ICE row with a known volume goes unscored). Verified: `./bin/test.sh` green (216 tests), `python build/build_data.py` clean, `verify_ui.py --page index --scenario grid` PASS — screenshot confirms the column renders with correct values (1.2l Turbo→2, 2.2l→5, 1.5l→3, 2.0l→4) color-graded green/red, blank on EV rows.
- [x] **#26** typ převodovky (transmission type) — new column — DONE 2026-07-05: derived payload-only column `Typ převodovky` (no new canonical column — deliberately avoids schema churn since all inputs already exist), computed in `build/build_data.py::derive_transmission_type()` from `Typ` + `Převodovka` + `Dvouspojková převodovka`: EV → always "Redukční (EV)"; ICE manual → "Manuální"; ICE automatic + DSG flag "Ano" → "Dvouspojková (DSG/DCT)"; ICE automatic otherwise → "Automatická" (hydraulic/CVT not distinguishable in the data — no guessing). `add_transmission_type_column()` inserts it right after `Dvouspojková převodovka`; wired into `write_payload()` alongside the existing `add_brand_model_columns()` call. Bug caught by the real-data value distribution during verification: `Převodovka` is spelled inconsistently across sources (sauto's `gearbox_cb.name` and mobile.de's German→Czech map write "Automatická"/"Manuální" natively, autodraft's own extractor writes short "Automat"/"Manual") — `site/transmissions.js`'s `AUTOMAT_VALUES`/`MANUAL_VALUES` already handled both spellings, so `derive_transmission_type()` was fixed to accept both too (an initial version keyed only on "Automat"/"Manual" and silently left 95% of ICE rows blank). Added to `site/app.js` COL_CONFIG (set-filter column next to "Dvouspojková převodovka", w:150) — `Převodovka`/`Dvouspojková převodovka` stay visible, not hidden. New `verify_ui.py --page index --scenario transmission-type-col` scrolls the column into view (it sits off the default viewport) since the default `grid` scenario screenshot doesn't reach it. Verified: `./bin/test.sh` green (213 tests, new `tests/test_build_data.py::DeriveTransmissionTypeTest`/`AddTransmissionTypeColumnTest`), `python build/build_data.py` clean (live distribution: 17168 Redukční (EV), 1375 Automatická, 1233 Dvouspojková (DSG/DCT), 935 Manuální, 43 blank/no-data), `verify_ui.py --page index --scenario {grid,transmission-type-col,stav-filter}` all PASS (screenshots confirm correct per-row values).
